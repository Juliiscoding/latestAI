{{ config(
    materialized='view',
    pre_hook=[
        "grant usage on schema {{ source('google_analytics_4', 'USER_ACQUISITION_FIRST_USER_SOURCE_MEDIUM_REPORT').database }}.{{ source('google_analytics_4', 'USER_ACQUISITION_FIRST_USER_SOURCE_MEDIUM_REPORT').schema }} to role MERCURIOS_FIVETRAN_USER",
        "grant select on {{ source('google_analytics_4', 'USER_ACQUISITION_FIRST_USER_SOURCE_MEDIUM_REPORT') }} to role MERCURIOS_FIVETRAN_USER",
        "grant select on {{ source('google_analytics_4', 'DEMOGRAPHIC_CITY_REPORT') }} to role MERCURIOS_FIVETRAN_USER"
    ]
) }}
...
with user_source_medium as (
    ...
    from {{ source('google_analytics_4', 'USER_ACQUISITION_FIRST_USER_SOURCE_MEDIUM_REPORT') }}
),
...
user_campaign as (
    ...
    from {{ source('google_analytics_4', 'USER_ACQUISITION_FIRST_USER_CAMPAIGN_REPORT') }}
    group by 1, 2, 3
),
