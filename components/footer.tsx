          ...
          <div>
            ...
            <div className="mt-4 flex space-x-4">
              <a href="https://twitter.com/mercuriosai" target="_blank" rel="noopener noreferrer" aria-label="Twitter">
                <svg className="h-5 w-5 text-white/70 hover:text-[#6ACBDF]" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <ul className="space-y-2">
              {companyLinks.map((link) => (
                <li key={link.title}>
                  <Link 
                    href={link.href} 
                    className="text-white/70 hover:text-[#6ACBDF] transition-colors text-sm"
                  >
                    {link.title}
                  </Link>
                </li>
        ...
        <div className="border-t border-white/10 mt-8 pt-6">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <p className="text-white/50 text-xs">
              © {new Date().getFullYear()} Mercurios.ai. Alle Rechte vorbehalten.
            </p>
