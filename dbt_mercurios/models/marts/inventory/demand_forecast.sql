{{
  config(
    materialized = 'table',
    cluster_by = ['article_id', 'forecast_date']
  )
}}
...
with sales_history as (
    ...
    from {{ ref('stg_prohandel__sales') }} s
    join {{ ref('stg_prohandel__articles') }} a on s.article_id = a.article_id
...
demand_forecast as (
    ...
    from {{ ref('stg_prohandel__articles') }} a
