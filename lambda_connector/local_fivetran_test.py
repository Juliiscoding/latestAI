#!/usr/bin/env python
"""
Local test script to simulate Fivetran requests to the Lambda function.
This script imports the Lambda function directly and tests it locally.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f"local_lambda_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ]
)
...
try:
    ...
    logger.error("Failed to import lambda_function.py. Make sure it exists in the current directory.")
    sys.exit(1)
...
def create_test_event(operation: str, entity: Optional[str] = None, 
    ...
    Returns:
        Dictionary with the event payload
    """
    event = {
        "agent": "fivetran",
        "version": 2,
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Test Lambda function locally with Fivetran requests')
    ...
    parser.add_argument('--output', type=str, default=f"local_lambda_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                        help='Output file for test results')
    return parser.parse_args()

def main():
    """Main function to run local Lambda tests."""
    args = parse_args()
    ...
    test_event = create_test_event(operation="test")
    ...
    schema_event = create_test_event(operation="schema")
    ...
    with open(args.output, 'w') as f:
        ...
        for result in results:
            if "event" in result and "secrets" in result["event"]:
                result["event"]["secrets"] = {"redacted": "for security"}
    ...
    # Print summary table
    print("\n" + "=" * 80)
    print("LOCAL LAMBDA FUNCTION TEST SUMMARY")
    print("=" * 80)
    print(tabulate(
        summary_rows,
